#!/usr/bin/env python3
"""
Classical KMeans DEGA planner + plotting + ROS2 output writer.

What this script does:
1) Loads 3D waypoints from a .pkl (Nx3).
2) Runs Classical KMeans clustering into num_uavs clusters.
3) Runs GA per cluster to produce optimized closed tours (return-to-start).
4) Saves a ROS2/Aerostack-consumable output pickle:
   - deckga_paths (list of Nx3 paths, in ORIGINAL coordinates)
   - raw_lengths, dega_lengths, centroids, offset_used, etc.
5) Shows the final combined optimized-path plot only if SHOW_PLOTS = True.
6) Optionally saves figures if --save_fig_dir is provided.

Run examples:
- Use latest offline dataset:
  python3 DEGA_Classical_Kmeans.py --points_pkl data/points/points_current.pkl --num_uavs 3

- Save figures:
  python3 DEGA_Classical_Kmeans.py --points_pkl data/points/points_current.pkl --save_fig_dir results/figs

- Write output explicitly:
  python3 DEGA_Classical_Kmeans.py --points_pkl data/points/points_current.pkl --out_pkl deckga_ros2/data/deckga_output.pkl
"""

# ---------------------------------------------------------------
# Set False to suppress all plt.show() calls (e.g. batch runs).
# Figures are still saved to disk when --save_fig_dir is given.
SHOW_PLOTS = False
# ---------------------------------------------------------------
...
from Classical_Kmeans import manual_kmeans_clustering
...
# ----------------------------
def main():
    repo_root = Path(__file__).resolve().parent.parent  # DEGA_Classical_Kmeans/ → repo root
    ...
    # Step 2: Run Classical KMeans clustering
    start_time_kmeans = time.time()
    clusters, centroids = manual_kmeans_clustering(points_shifted, num_uavs)
    elapsed_time_kmeans = time.time() - start_time_kmeans

    print("\n--- Classical KMeans Output ---")
    print("Centroids:\n", centroids)
    print(f"KMeans completed in {elapsed_time_kmeans:.3f} s")

    # Optional: KMeans plot
    if (not args.no_plot) and SHOW_PLOTS and args.plot_kmeans:
        fig_kmeans = plt.figure()
        ax_kmeans = fig_kmeans.add_subplot(111, projection="3d")
        ax_kmeans.set_title("Classical KMeans Clustering (shifted space)")
    ...
    out = {
        ...
        "ga_time_s": elapsed_time_ga,   # total GA wall time (100k iters, no scaling needed)
    ...
    if (not args.no_plot) and SHOW_PLOTS:
        fig_combined = plt.figure(figsize=(4.5, 3.2), dpi=200)
        ax = fig_combined.add_subplot(111, projection="3d")
        ax.set_title("Optimised UAV Paths (Classical KMeans DEGA)", fontsize=10)
        ...
        # ax.legend()  # optional
        maybe_save_fig(fig_combined, save_dir, "classical_kmeans_dega_optimized_paths_combined")
        plt.show()
        ...
        if args.plot_per_uav:
            for i, path in enumerate(deckga_paths):
                ...
                ax2.set_zlabel("Z")
                ax2.legend()
                maybe_save_fig(fig, save_dir, f"classical_kmeans_dega_uav{i+1}_path")
                plt.show()

    # Summary timing
    print("\n--- Timing Summary ---")
    print(f"KMeans Cluster Time       : {elapsed_time_kmeans:.3f} s")
    print(f"Classical KMeans GA Time  : {elapsed_time_ga:.3f} s")
