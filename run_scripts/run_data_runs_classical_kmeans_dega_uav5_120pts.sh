...
# Usage: ./run_data_runs_classical_kmeans_dega_uav5_120pts.sh <start_run> <end_run>
...
TMUX_SESSIONS=("drone0" "drone1" "drone2" "drone3" "drone4")
...
REQUIRED_ACTIONS=(
    ...
    "/drone4/TakeoffBehavior" "/drone4/GoToBehavior" "/drone4/LandBehavior" "/drone4/FollowPathBehavior"
)

wait_for_ready() {
    local timeout_s=300 interval=5 elapsed=0 echo_timeout=4
...
wait_until_clean() {
    local timeout_s=60 interval=2 elapsed=0
    ...
    while true; do
        ...
        if (( elapsed >= timeout_s )); then
            echo "[WARN] Processes still alive after ${timeout_s}s — force-killing..."
...
do_cleanup() {
    ...
    rm -rf /tmp/.gz* /tmp/gz*                      2>/dev/null || true
    ros2 daemon start 2>/dev/null || true
    wait_until_clean
    echo "[cleanup] Settling 25s..."
    sleep 25
    echo "--- Cleanup done ---"
}
...
for N in $(seq "$START_RUN" "$END_RUN"); do
    ...
    echo "[run $N] Writing 5-UAV world config..."
    cat > "$PROJECT_GAZEBO/config/world_swarm_5.yaml" << 'YAML'
  ...
  - model_type: "quadrotor_base"
    ...
    payload:
      - model_type: "gps"
        model_name: "gps"
YAML
    cp "$PROJECT_GAZEBO/config/world_swarm_5.yaml" "$PROJECT_GAZEBO/config/world_swarm.yaml"
    grep "model_name" "$PROJECT_GAZEBO/config/world_swarm.yaml"

    echo "[run $N] Launching Gazebo + Aerostack2 (5-UAV config)..."
    (
        set +eu
        source "$ROS2_SETUP"
        source "$AS2_SETUP"
    ...
    echo "[run $N] Sleeping 70s for boot (5-UAV needs more time)..."
    sleep 70
    ...
    python3 deckga_ros2/drone_pairwise_distance_logger.py \
        ...
        --run_tag "ex${N}_120pts_uav5_classical_kmeans_dega_pairwise_run_${N}" &
    LOGGER_PID=$!
    ...
    python3 deckga_ros2/deckga_execute_antarctica.py \
        --deckga_pkl "deckga_ros2/data/classical_kmeans_dega_output_antarctica_120_uav5_ex${N}.pkl" \
        --namespaces drone0,drone1,drone2,drone3,drone4 \
        --coord_mode auto \
        ...
        --wp_settle_s 0.3 \
        --log_dir results_antarctica_csv \
        --run_tag "ex${N}_120pts_uav5_classical_kmeans_dega_run_${N}"
    EXEC_EXIT=$?
    set -e
