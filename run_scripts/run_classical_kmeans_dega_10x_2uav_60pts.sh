...
TOTAL=10
RESULTS="results_csv/classical_kmeans_dega_2uavs_60_points_10_runs_results.txt"
...
{
    echo "Classical KMeans DEGA | 2 UAVs | 60 Waypoints | 10 Runs Results"
...
for i in $(seq 1 $TOTAL); do
    echo "=== Running Classical KMeans DEGA 60pts 2-UAV Run $i/$TOTAL ==="

    OUT=$(python3 "DEGA_Classical_Kmeans/DEGA_Classical_Kmeans.py" \
        --points_pkl "$POINTS_PKL" \
        --num_uavs 2 \
        --start_points="$START_POINTS" \
        --out_pkl "$OUT_DIR/classical_kmeans_dega_output_antarctica_60_uav2_ex${i}.pkl" \
...
for i in range(1, 11):
    pkl_path = out_dir / f"classical_kmeans_dega_output_antarctica_60_uav2_ex{i}.pkl"
