...
# Usage: ./run_data_runs_trad_ga_dc_uav3_90pts.sh <start_run> <end_run>
# Example: ./run_data_runs_trad_ga_dc_uav3_90pts.sh 1 10
...
for N in $(seq "$START_RUN" "$END_RUN"); do
    ...
    python3 deckga_ros2/drone_pairwise_distance_logger.py \
        ...
        --run_tag "ex${N}_90pts_uav3_trad_ga_dc_pairwise_run_${N}" &
    LOGGER_PID=$!
    echo "[run $N] Pairwise logger started (PID $LOGGER_PID). Sleeping 3s..."
    sleep 3

    # ------------------------------------------------------------------
    # Step 4: Run Traditional GA D&C executor (foreground — blocks until mission done)
    # ------------------------------------------------------------------
    echo "[run $N] Running Traditional GA D&C executor (ex${N} PKL)..."
    set +e
    python3 deckga_ros2/deckga_execute_antarctica.py \
        --deckga_pkl "deckga_ros2/data/traditional_ga_divide_conquer_output_antarctica_90_uav3_ex${N}.pkl" \
        ...
        --wp_settle_s 0.3 \
        --log_dir results_antarctica_csv \
        --run_tag "ex${N}_90pts_uav3_trad_ga_dc_run_${N}"
    EXEC_EXIT=$?
    set -e
