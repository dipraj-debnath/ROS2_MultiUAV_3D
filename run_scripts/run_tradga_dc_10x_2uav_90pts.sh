...
TOTAL=10
RESULTS="results_csv/trad_ga_dc_2uavs_90_points_10_runs_results.txt"
...
{
    echo "Traditional GA D&C | 2 UAVs | 90 Waypoints | 10 Runs Results"
...
for i in $(seq 1 $TOTAL); do
    echo "=== Running Traditional GA D&C 90pts 2-UAV Run $i/$TOTAL ==="

    OUT=$(python3 "DEGA_Divide & Conquer/Traditional_GA_Divide & Conquer.py" \
        --points_pkl "$POINTS_PKL" \
        --num_uavs 2 \
        --start_points="$START_POINTS" \
        --out_pkl "$OUT_DIR/traditional_ga_divide_conquer_output_antarctica_90_uav2_ex${i}.pkl" \
...
for i in range(1, 11):
    pkl_path = out_dir / f"traditional_ga_divide_conquer_output_antarctica_90_uav2_ex{i}.pkl"
