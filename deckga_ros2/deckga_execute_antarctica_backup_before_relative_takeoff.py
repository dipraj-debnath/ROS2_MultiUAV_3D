...
def main():
    ...
    ap.add_argument(
        "--skip_takeoff",
        action="store_true",
        help="Skip TakeoffBehavior and use the first waypoint handshake as the initial climb/takeoff.",
    )
    ...
    if args.skip_takeoff and not args.first_wp_wait:
        raise ValueError("--skip_takeoff requires --first_wp_wait so each drone moves to its first waypoint safely.")
    ...
    try:
        ...
        if args.skip_takeoff:
            print("Skipping TakeoffBehavior.")
            print("Drones are armed/offboard and will move directly to their first waypoint.")
            print("This avoids the unwanted vertical takeoff climb in the Antarctica world.")
        else:
