...
# Usage: ./run_data_runs_uav5_60pts.sh <start_run> <end_run>
# Example: ./run_data_runs_uav5_60pts.sh 1 10
...
for N in $(seq "$START_RUN" "$END_RUN"); do
    ...
    # ------------------------------------------------------------------
    # Step 1: Activate 5-UAV 60pt swarm config, then launch Gazebo + Aerostack2
    # ------------------------------------------------------------------
...
YAML
    ...
    python3 deckga_ros2/drone_pairwise_distance_logger.py \
        ...
        --run_tag "ex${N}_60pts_uav5_pairwise_distance_run_${N}" &
    LOGGER_PID=$!
    ...
    python3 deckga_ros2/deckga_execute_antarctica.py \
        --deckga_pkl "deckga_ros2/data/deckga_output_antarctica_60_uav5_ex${N}.pkl" \
        ...
        --wp_settle_s 0.3 \
        --log_dir results_antarctica_csv \
        --run_tag "ex${N}_60pts_uav5_deckga_run_${N}"
    EXEC_EXIT=$?
    set -e
