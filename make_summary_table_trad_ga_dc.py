...
parser = argparse.ArgumentParser(
    description="Build a summary table for a Traditional GA D&C benchmark experiment."
)
...
OUT_FILE    = os.path.join(OUT_DIR, f"summary_table_trad_ga_dc_uav{UAVS}_{WPTS}pts.txt")
...
for n in range(1, NUM_RUNS + 1):
    exec_file = find_csv(
        f"*ex{n}_{WPTS}pts_uav{UAVS}_trad_ga_dc_run_{n}*summary.csv"
    )
    pairwise_file = find_csv(
        f"*ex{n}_{WPTS}pts_uav{UAVS}_trad_ga_dc_pairwise_run_{n}*summary.csv"
    )

    row = {"Run": str(n)}
    for col in DECKGA_COLS:
        row[col] = read_cell(exec_file, col)
