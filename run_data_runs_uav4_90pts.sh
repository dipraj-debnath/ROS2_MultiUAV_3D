...
# Usage: ./run_data_runs_uav4_90pts.sh <start_run> <end_run>
# Example: ./run_data_runs_uav4_90pts.sh 1 10
...
for N in $(seq "$START_RUN" "$END_RUN"); do
    ...
    # ------------------------------------------------------------------
    # Step 1: Activate 4-UAV 90pt swarm config, then launch Gazebo + Aerostack2
    # ------------------------------------------------------------------
    echo "[run $N] Writing 4-UAV 90pt world config to world_swarm_4.yaml..."
...
YAML
    ...
    python3 deckga_ros2/drone_pairwise_distance_logger.py \
        ...
        --run_tag "ex${N}_90pts_uav4_pairwise_distance_run_${N}" &
    LOGGER_PID=$!
    ...
    python3 deckga_ros2/deckga_execute_antarctica.py \
        --deckga_pkl "deckga_ros2/data/deckga_output_antarctica_90_uav4_ex${N}.pkl" \
        ...
        --wp_settle_s 0.3 \
        --log_dir results_antarctica_csv \
        --run_tag "ex${N}_90pts_uav4_deckga_run_${N}"
    EXEC_EXIT=$?
    set -e
